...
def convert_new_mask(mask):
    mask = mask.squeeze(1)  # 现在尺寸为 [1, 512, 512]

    # 使用 one_hot 将 mask 转换为独热编码，并将结果转换为浮点类型
    one_hot_mask = F.one_hot(mask, num_classes=2).float()  # 结果尺寸为 [1, 512, 512, 2]

    # 重新调整张量的形状为 [1, 2, 512, 512]
...
def convert_one_hot(oh, max_obj):
    ...
    mask = torch.argmax(oh, dim=1, keepdim=True)
