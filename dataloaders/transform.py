...
class Transpose(object):
    ...
    def __call__(self, imgs, annos, use_image):
        ...
        # reshaped_imgs = imgs[:, :, np.newaxis]
        # reshaped_imgs = [img[:, :, np.newaxis] for img in imgs]
        H, W, _ = imgs[0].shape  # H, W, _ = imgs[0].shape 但是imgs[0]只有两个属性，长和宽
        if H < W:
            return imgs, annos
        else:
            timgs = [np.transpose(img, [1, 0, 2]) for img in
                     imgs]  # timgs = [np.transpose(img, [1, 0, 2]) for img in imgs]
...
class Rescale(object):
    ...
    def __call__(self, imgs, annos, use_image):
        ...
        for id, img in enumerate(imgs):
            ...
            # new_channels = 3
            # rescaled_img = np.repeat(rescaled_img, new_channels, axis=-1)
            canvas[pad_t:pad_t + height, pad_l:pad_l + width, :] = rescaled_img
            # canvas[pad_t:pad_t + height, pad_l:pad_l + width, :] = img
            imgs[id] = canvas
...
class Normalize(object):
    ...
    def __call__(self, imgs, annos, use_image):
        for id, img in enumerate(imgs):
            # img = np.clip(img, 170, 190)
            mean = np.mean(img)
            std = np.std(img)
            imgs[id] = (img - mean) / std
...
class ScaleIntensityRanged(object):
    def __init__(self):
        self.min = np.float32(63)
        self.max = np.float32(163)
        self.deviation = self.max - self.min

    def __call__(self, imgs, annos, use_image):
        for id, img in enumerate(imgs):
            imgs[id] = (np.clip(img, self.min, self.max) - self.min) / self.deviation
...
#     def __init__(self):
#         self.min = np.float32(71)
#         self.max = np.float32(156)
