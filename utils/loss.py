...
def cross_entropy_loss(pred, mask, num_object, bootstrap=0.4):
    ...
    loss = torch.sum(pred[:, 0:num_object] * mask[:, 0:num_object], dim=1).view(N, -1)
...
def cross_entropy_loss_1(pred, mask, num_object, bootstrap=0.4,gamma=2):
    mask_log = torch.log(mask[:, 1, :, :])
    loss = -mask[:, 1, :, :] * ((1 - pred[:, 1, :, :]) ** gamma) * torch.log(pred[:, 1, :, :])
    return loss.mean()


def focal_loss(pred, mask, num_object, bootstrap=0.4, alpha=0.2, gamma=2):
    mask_log = torch.log(mask[:, 1, :, :])
    loss = -alpha * mask[:, 0, :, :] * ((1 - pred[:, 0, :, :]) ** gamma) * torch.log(pred[:, 0, :, :]) - (
            1 - alpha) * mask[:, 1, :, :] * ((1 - pred[:, 1, :, :]) ** gamma) * torch.log(pred[:, 1, :, :])
    return loss.mean()


def my_loss(pred, mask):
    # pred: [N x K x H x W]
    # mask: [N x K x H x W] one-hot encoded
    N, _, H, W = mask.shape
